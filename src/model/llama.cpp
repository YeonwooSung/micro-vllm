...
private:
    ...
    void slot_alloc(LlamaSlot &s, int t_max) {
        const int H = std::max(cfg_.hidden, 1);
        const int nkv = std::max(cfg_.n_kv_heads, 1);
        const int hd = std::max(cfg_.head_dim, 1);
        const int kvd = nkv * hd;
    ...
    void slot_ensure_t(LlamaSlot &s, int t_max) {
        const int Tmax = std::max(t_max, 1);
        const int L = std::max(cfg_.n_layers, 0);
    ...
    void slot_step(LlamaSlot &s, int token) {
        const int H = std::max(cfg_.hidden, 1);
        const int I = std::max(cfg_.dense_intermediate, 1);
        const int nq = std::max(cfg_.n_q_heads, 1);
        const int nkv = std::max(cfg_.n_kv_heads, 1);
    ...
    void slot_prefill(LlamaSlot &s, const std::vector<int> &ids, int start) {
        if (start < 0)
            start = 0;
    ...
    int slot_sample(LlamaSlot &s, const GenParams &gp, uint64_t *rng, const uint8_t *allow) {
        const int H = std::max(cfg_.hidden, 1);
