...
class InklingEngine final : public FamilyEngine {
public:
    Family family() const override { return Family::Inkling; }
    ...
    int export_kv_rows(int slot, int pos0, int n, KvPersistRecord *rows) const override {
        if (!rows || n <= 0 || slot < 0 || slot >= kMaxKvSlots)
            return 0;
        const InklingSlot &sl = slots_[slot];
        if (!sl.have)
            return 0;
        const int nkv = std::max(cfg_.n_kv_heads, 1);
        const int hd = std::max(cfg_.head_dim, 1);
...
private:
    ...
    void slot_alloc(InklingSlot &s, int t_max) {
        const int H = std::max(cfg_.hidden, 1);
        const int nkv = std::max(cfg_.n_kv_heads, 1);
        const int hd = std::max(cfg_.head_dim, 1);
        const int kvd = nkv * hd;
    ...
    void slot_ensure_t(InklingSlot &s, int t_max) {
        const int Tmax = std::max(t_max, 1);
        const int L = std::max(cfg_.n_layers, 0);
    ...
    void slot_step(InklingSlot &s, int token) {
        const int H = std::max(cfg_.hidden, 1);
        const int I = std::max(cfg_.dense_intermediate, 1);
        const int nq = std::max(cfg_.n_q_heads, 1);
        const int nkv = std::max(cfg_.n_kv_heads, 1);
    ...
    void slot_prefill(InklingSlot &s, const std::vector<int> &ids, int start) {
        if (start < 0)
            start = 0;
    ...
    int slot_sample(InklingSlot &s, const GenParams &gp, uint64_t *rng, const uint8_t *allow) {
        const int H = std::max(cfg_.hidden, 1);
    ...
    void infer_heads(const io::StTensor *q, const io::StTensor *k) {
        ...
        if (cfg_.n_q_heads > 0 && q_rows % cfg_.n_q_heads == 0 &&
            ...
            return;
        // Tiny fixtures: prefer more heads. Full-size Llama: 64/128.
...
std::unique_ptr<FamilyEngine> make_inkling() { return std::make_unique<InklingEngine>(); }
