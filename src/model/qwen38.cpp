...
class Qwen38Engine final : public FamilyEngine {
public:
    ...
    int export_kv_rows(int slot, int pos0, int n, KvPersistRecord *rows) const override {
        if (!rows || n <= 0 || slot < 0 || slot >= kMaxKvSlots)
            return 0;
        const Qwen38Slot &sl = slots_[slot];
        if (!sl.have)
            return 0;
        const int nkv = std::max(cfg_.n_kv_heads, 1);
        const int hd = std::max(cfg_.head_dim, 1);
...
private:
    ...
    void slot_ensure_t(Qwen38Slot &s, int t_max) {
        const int Tmax = std::max(t_max, 1);
        const int L = std::max(cfg_.n_layers, 0);
    ...
    void slot_step(Qwen38Slot &s, int token) {
        const int H = std::max(cfg_.hidden, 1);
        const int I = std::max(cfg_.dense_intermediate, 1);
        const int nq = std::max(cfg_.n_q_heads, 1);
        const int nkv = std::max(cfg_.n_kv_heads, 1);
    ...
    void slot_prefill(Qwen38Slot &s, const std::vector<int> &ids, int start) {
        if (start < 0)
            start = 0;
    ...
    int slot_sample(Qwen38Slot &s, const GenParams &gp, uint64_t *rng, const uint8_t *allow) {
        const int H = std::max(cfg_.hidden, 1);
...
std::unique_ptr<FamilyEngine> make_qwen38() { return std::make_unique<Qwen38Engine>(); }
