...
constexpr char kMagic[8] = {'C', 'O', 'L', 'I', 'K', 'V', '2', '\0'};
constexpr int32_t kFormatKv8 = 1;
constexpr int64_t kPrefixBytes = 8 + 8 * 4;
constexpr int64_t kNrecOffset = 8 + 6 * 4;
constexpr float kScaleMax = 3.4e38f;
...
// Official kv8_sanitize: invalid scale → zeros + identity scale.
bool scale_ok(float sc) { return sc > 0.f && sc < kScaleMax; }
...
bool rec_bytes_for(int n_layers, int kv_lora, int qk_rope, int n_index, int index_hd,
    ...
    // token + n_layers * (L[kv_lora] + Lscale + R[qk_rope] + Rscale) + I
    const int64_t layer = static_cast<int64_t>(kv_lora) + static_cast<int64_t>(qk_rope) + 8;
...
Status KvPersistV2::normalize_config(const KvPersistConfig &in, std::string &err) {
...
KvPersistV2::~KvPersistV2() { close(); }

Status KvPersistV2::flush_sync(std::string &err) const {
    if (!fp_) {
...
Status KvPersistV2::seek_abs(int64_t off, std::string &err) const {
    if (!fp_ || off < 0) {
...
bool KvPersistV2::header_matches(const int32_t hdr[8]) const {
...
bool KvPersistV2::read_prefix(int32_t hdr[8]) const {
    char magic[8];
...
int KvPersistV2::physical_nrec() const {
    if (!fp_ || rec_bytes_ <= 0)
        return 0;
...
Status KvPersistV2::write_nrec(int nrec, std::string &err) {
...
Status KvPersistV2::create_empty(std::string &err) {
    if (fp_) {
        std::fclose(fp_);
        fp_ = nullptr;
    }
...
Status KvPersistV2::ensure_writable(std::string &err) {
    if (!open_) {
        err = "kv persist not open";
        return Status::InvalidArgument;
    }
...
Status KvPersistV2::open(const std::string &path, const KvPersistConfig &cfg, std::string &err) {
    close();
    if (path.empty()) {
...
bool KvPersistV2::record_ok(const KvPersistRecord &rec, std::string &err) const {
...
void KvPersistV2::pack_record(uint8_t *dst, int token, const KvPersistRecord &rec) const {
    ...
    for (int i = 0; i < nl; ++i) {
        const float lsc =
            kv_fp8_quant_row(rec.L.data() + static_cast<size_t>(i) * static_cast<size_t>(kl), dst,
                             kl);
        dst += static_cast<size_t>(kl);
        std::memcpy(dst, &lsc, sizeof(lsc));
        dst += sizeof(lsc);

        const float rsc =
            kv_fp8_quant_row(rec.R.data() + static_cast<size_t>(i) * static_cast<size_t>(qr), dst,
                             qr);
        dst += static_cast<size_t>(qr);
        std::memcpy(dst, &rsc, sizeof(rsc));
        dst += sizeof(rsc);
    }
    size_t ioff = 0;
    if (ih > 0) {
...
void KvPersistV2::unpack_record(const uint8_t *src, KvPersistRecord &rec) const {
    int32_t tk = 0;
    ...
    for (int i = 0; i < nl; ++i) {
        float *Ldst = rec.L.data() + static_cast<size_t>(i) * static_cast<size_t>(kl);
        const uint8_t *Lq = src;
        src += static_cast<size_t>(kl);
        float lsc = 0.f;
        std::memcpy(&lsc, src, sizeof(lsc));
        src += sizeof(lsc);
        if (scale_ok(lsc))
            kv_fp8_dequant_row(Lq, lsc, Ldst, kl);
        // else zeros already; NaN e4m3 codes decode to 0 via kv_fp8_lut

        float *Rdst = rec.R.data() + static_cast<size_t>(i) * static_cast<size_t>(qr);
        const uint8_t *Rq = src;
        src += static_cast<size_t>(qr);
        float rsc = 0.f;
        std::memcpy(&rsc, src, sizeof(rsc));
        src += sizeof(rsc);
        if (scale_ok(rsc))
            kv_fp8_dequant_row(Rq, rsc, Rdst, qr);
    }
    size_t ioff = 0;
    if (ih > 0) {
        for (int i = 0; i < nl; ++i) {
...
int KvPersistV2::load(std::vector<int> &hist, std::vector<KvPersistRecord> *rows, std::string &err) {
    hist.clear();
    if (rows)
        rows->clear();
    if (!open_ || !fp_ || stale_)
...
Status KvPersistV2::append(const int *hist, int hist_n, const KvPersistRecord *recs, int rec_n,
                           std::string &err) {
    if (!open_) {
        err = "kv persist not open";
        return Status::InvalidArgument;
...
Status KvPersistV2::truncate(int nrec, std::string &err) {
    if (!open_) {
        err = "kv persist not open";
        return Status::InvalidArgument;
    }
...
Status KvPersistV2::reset(std::string &err) { return truncate(0, err); }
