"""
Vendored from colibri c/tools/convert_glm53.py for the micro-vllm host.

Official GLM-5.3 HF dumps are FP8. The host engine only loads int4-g64
U8 + `.qs` expert containers. Run this before `micro-vllm smoke/load`
on a raw Flash snapshot:

  python3 python/convert_glm53.py --indir /path/raw --outdir /path/glm53_i4

Kimi K3 official MXFP4 shards load without a converter. Optional
`k3_repack.py` lives in colibri and only speeds cold start.
